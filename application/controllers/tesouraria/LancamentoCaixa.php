...
class LancamentoCaixa extends My_Controller {
	...
	public function index(){
		...
		$plano 		= $this->planocontas_model->get();

		if($plano) {
			foreach ($plano as $r) {
				$data['BLC_PLANO'] [] = array(
					"COD" 		=> $r->PlanoID,
					"CODIGO" 	=> $r->CodigoContabil,
					"PLANO"		=> $r->DescricaoPlano
				);
			}
		}
  ...
  //       $this->setURL ($data);
		...
		$this->parser->parse('tesouraria/LancamentoCaixa_form',$data);
	...
	public function PlanoConta($redirect) {
		...
		$plano = "ativo";
		$sql = "select * from vw_planocontas where nat1 = '$plano'";

		$result = $this->model_main->get_query ($sql);

		$EliminaDados = array();
		
		for ( $i = 0; $i < count($result); $i++ ) {
            $EliminaDados[] = get_object_vars($result[$i]);
        }

        $dados = array();
        $dados[$plano] = array();

  		for ( $i = 0; $i < count($EliminaDados); $i++ )
  		{
  			$dados[$plano][$EliminaDados[$i]['nat2']] = array();

  			for ( $x = 0; $x < count($EliminaDados); $x++ )
  			{
  				if($EliminaDados[$i]['nat2'] == $EliminaDados[$x]['nat2'])
  				{
  					$dados[$plano][$EliminaDados[$i]['nat2']][$EliminaDados[$i]['nat3']] = array();
  				} else {
  					$dados[$plano][$EliminaDados[$i]['nat2']][$EliminaDados[$i]['nat3']] = array();
  				}
        	}
